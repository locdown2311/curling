...
(function () {
    ...
    class GameManager {
        constructor() {
            ...
            this.currentEnd = 1;
            this.scores = [0, 0];
            this.stonesThrown = [0, 0]; // per-end count
            this.currentTeam = 0;
            this.hammer = 1; // team with last stone advantage (yellow starts with hammer)
            this.state = 'lobby'; // lobby | aiming | launching | sweeping | waiting | end_over | game_over
            ...
            // Pebble pattern seed (fixed per game)
            this.pebbleSeed = [];
        ...
        setupLobby() {
            ...
            document.querySelectorAll('.end-btn').forEach(btn => {
                btn.addEventListener('click', () => {
                    document.querySelectorAll('.end-btn').forEach(b => b.classList.remove('active'));
                    btn.classList.add('active');
                    this.totalEnds = parseInt(btn.dataset.ends);
                });
            });
        ...
        startGame() {
            this.team1Name = document.getElementById('team1Name').value || 'Equipe Vermelha';
            this.team2Name = document.getElementById('team2Name').value || 'Equipe Amarela';
            ...
            this.scores = [0, 0];
            this.currentEnd = 1;
            this.hammer = 1; // yellow has hammer first
        ...
        startEnd() {
            ...
            // Generate rough ice patches for this end
            this.physics.generateRoughZones();

            // Team without hammer throws first
        ...
        endEnd() {
            ...
            // Show end summary
            document.getElementById('modalTitle').textContent = `Fim do End ${this.currentEnd}`;
            ...
            document.getElementById('modalTeam1Points').textContent = `+${endPoints[0]}`;
            document.getElementById('modalTeam2Points').textContent = `+${endPoints[1]}`;
            ...
            if (this.currentEnd > this.totalEnds) {
                // Game over
                document.getElementById('nextEndBtn').querySelector('span').textContent = 'Ver Resultado';
                document.getElementById('nextEndBtn').onclick = () => {
                    document.getElementById('endModal').classList.remove('visible');
                    this.showGameOver();
                };
            } else {
                document.getElementById('nextEndBtn').querySelector('span').textContent = 'Próximo End';
                document.getElementById('nextEndBtn').onclick = () => {
                    document.getElementById('endModal').classList.remove('visible');
                    this.startEnd();
                };
        ...
        updateUI() {
            ...
            if (this.currentTeam === 0) {
                ...
                turnText.textContent = `Vez: ${this.team1Name}`;
            } else {
                turnDot.style.background = 'var(--yellow-team)';
                turnText.textContent = `Vez: ${this.team2Name}`;
            }
        ...
        update() {
            ...
            if (this.state === 'aiming') {
                ...
                if (elapsed >= AIM_TIME_LIMIT) {
                    // Time's up — lose this stone
                    this.stonesThrown[this.currentTeam]++;
                    this.isDragging = false;
                    this.aimTarget = null;
                    this.power = 0;
